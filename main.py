...
def main():
    num_processes = random.randint(1, 100)  # Ensure at least 1 process
    ...
    def run_schedulers(env, psjf_scheduler, round_robin_scheduler, sjf_scheduler, fcfs_scheduler):
        ...
        print("RR started at:", env.now)
        yield env.process(round_robin_scheduler.run())
        print("RR ended at:", env.now)
        
        print("NPSJF started at:", env.now)
        yield env.process(sjf_scheduler.run())
        print("NPSJF ended at:", env.now)
        
        print("FCFS started at:", env.now)
        yield env.process(fcfs_scheduler.run())
        print("FCFS ended at:", env.now)

    env.process(run_schedulers(env, psjf_scheduler, round_robin_scheduler, sjf_scheduler, fcfs_scheduler))
    env.run()
